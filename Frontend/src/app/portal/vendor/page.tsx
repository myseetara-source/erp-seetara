...
export default function VendorPortalDashboard() {
  ...
  const loadDashboard = useCallback(async () => {
    ...
    try {
      ...
      if (!token) {
        router.push('/portal/vendor/login');
        return;
      }
      ...
      if (error.response?.status === 401 || error.response?.status === 403) {
        ...
        router.push('/portal/vendor/login?error=session_expired');
        return;
      }
                ...
                {data.recentTransactions.map((txn) => (
                  <TableRow key={txn.id}>
                    ...
                    <TableCell className={cn(
                      ...
                      txn.type === 'payment' || txn.type === 'vendor_payment' ? 'text-green-600' : 
                      txn.type === 'purchase_return' ? 'text-red-600' : 'text-gray-900'
                    )}>
                      {txn.type === 'payment' || txn.type === 'vendor_payment' ? '+' : 
                       txn.type === 'purchase_return' ? '-' : ''}{formatCurrency(Math.abs(txn.amount))}
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
