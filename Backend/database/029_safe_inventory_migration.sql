-- =============================================================================
-- MIGRATION 029: SAFE INVENTORY SYSTEM (No Dependencies)
-- =============================================================================
-- This is a CLEAN migration that creates everything from scratch
-- Safe to run multiple times
-- =============================================================================

-- =============================================================================
-- STEP 1: CREATE/UPDATE STOCK_MOVEMENTS TABLE
-- =============================================================================
...
RAISE NOTICE '✅ Step 1: stock_movements table created';

-- =============================================================================
-- STEP 2: PERFORMANCE INDEXES
-- =============================================================================

CREATE INDEX IF NOT EXISTS idx_inv_tx_vendor_type ON inventory_transactions(vendor_id, transaction_type);
...
RAISE NOTICE '✅ Step 2: Performance indexes created';

-- =============================================================================
-- STEP 3: MASTER STOCK SYNC TRIGGER (Simplified)
-- =============================================================================
  ...
  IF NEW.status = 'approved' AND (TG_OP = 'INSERT' OR OLD.status IS DISTINCT FROM 'approved') THEN
    ...
    LOOP
      ...
      IF delta != 0 THEN
        ...
        VALUES (item.variant_id, ABS(delta), mv_type, 'inventory_tx', NEW.id, 
...
RAISE NOTICE '✅ Step 3: Stock sync trigger created';
...
BEGIN
  SELECT json_build_object(
    ...
    'critical_stock', (
      SELECT json_build_object(
        ...
        'items', COALESCE(json_agg(json_build_object(
          'id', pv.id, 'sku', pv.sku, 'product_name', p.name, 
          'current_stock', pv.current_stock, 'threshold', 10
    ...
    'damage_loss', (
      ...
      WHERE transaction_type = 'damage' AND status = 'approved' AND created_at >= m_start
    ),
    ...
    'stock_trend', COALESCE((
      SELECT json_agg(json_build_object('day', d::date, 'net_change', 
        COALESCE((SELECT SUM(CASE WHEN movement_type = 'in' THEN quantity ELSE -quantity END) 
                  FROM stock_movements WHERE created_at::date = d::date), 0)
      ) ORDER BY d)
...
RAISE NOTICE '✅ Step 4: Dashboard RPC created';
...
BEGIN
  RETURN json_build_object(
    ...
    'data', COALESCE((
      ...
      FROM product_variants pv
      JOIN products p ON p.id = pv.product_id
      WHERE pv.is_active = true AND (p_product IS NULL OR p.id = p_product)
      LIMIT p_limit
    ), '[]'),
    'generated_at', NOW()
  );
...
RAISE NOTICE '✅ Step 5: Movement report RPC created';
...
BEGIN
  ...
  IF r IS NOT NULL THEN
    ...
    RAISE NOTICE '═══════════════════════════════════════════════════';
    RAISE NOTICE '  ✓ stock_movements table ready';
